...
describe('Date filter component', function () {
    ...
    beforeEach(inject(function ($rootScope, $componentController) {
        scope = $rootScope.$new();
        ctrl = $componentController('wildcardFilter', {$scope: scope}, {
            wildcardValues: 'asdsad',
    ...
    it('should init model', function () {
        ctrl.$onInit();
        expect(ctrl.values).toBe(ctrl.wildcardValues);
    });

    it('should call onApply with filters', function () {
        spyOn(ctrl, 'onApply');
        ctrl.applyFilter();
        expect(ctrl.onApply).toHaveBeenCalledWith({values: ctrl.values});
    });
