...
function debugout() {
	...
	self.logFilename = 'debugout.json'; // filename of log downloaded with downloadLog()
    self.lineLength = 240;
    self.insertLineBreak = false; // if true, inserts \n after 'lineLength' chars, else, deletes chars for 'lineLength' to end of line.
    ...
    this.splitSlice = function splitSlice(str, len) {
        var ret = [ ];
	...
	*/
	...
	this.log = function(obj) {
		...
		if (type != null && self.recordLogs) {
            ...
            if (self.insertLineBreak)
            {
                var arr = this.splitSlice(addition, self.lineLength);
                self.output += arr.join('\n') + '\n';
            } else {
	...
	*/
	...
	self.output += '{ \"log start\": \" ---- Session started: '+self.startTime+' ---- \",';
}
