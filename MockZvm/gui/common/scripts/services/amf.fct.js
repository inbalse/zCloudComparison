...
angular.module('zvmApp.services')
    .factory('amf', ['amf2', function (amf2) {
        /***
        ...
        var amf = {
            ...
            ActionMessage: function () {
                return {
                    ...
                    version: 3,
                    headers: [
                        {name: "mobile", mustUnderstand: false, data: true}
                    ],
                    bodies: []
                };
            },
...
//                                    var message = deserializer.readMessage();
                                    var messages = amf2.decodeAMF(new Uint8Array(this.response)).messages;
//                                    for (var bodyIndex in message.bodies) {
                                    messages.forEach(function(message) {
//                                        var body = message.bodies[bodyIndex];
                                        if (message.body && message.body.hasOwnProperty('faultCode')){// ._explicitType == "flex.messaging.messages.ErrorMessage") {
                                            onStatus({faultCode: message.body.faultCode, faultDetail: message.body.faultDetail, faultString: message.body.faultString});
                                        } else {
                                            if (message.targetURI && message.targetURI.indexOf("/onResult") > -1) {
                                                if (message.targetURI == "/1/onResult") {
//                                                    this.parent.clientId = body.data.clientId;
                                                    //this.parent.clientId = body.body.clientId;
