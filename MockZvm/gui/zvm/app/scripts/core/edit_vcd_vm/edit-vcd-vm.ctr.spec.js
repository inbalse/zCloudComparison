...
describe('editVcdVmControllerTest', function () {
    var controller, scope, factory, vos, service, enums, translate;
    beforeEach(module('zvmTest'));

    beforeEach(inject(function ($controller, $rootScope, $translate, _vos_, _enums_, _editVcdVmFactory_, _objectTransformHelpersService_, _createVPGModel_) {
        scope = $rootScope.$new();
        translate = $translate;
        enums = _enums_;
        factory = _editVcdVmFactory_;
        vos = _vos_;
        ...
        spyOn(factory, 'getVCDPotentialVirtualDatacenterStorageProfiles').and.callThrough();

        var data = {defaultVpgSettings: {Id: {}, Config: {Configuration: {ManageJournalSettings: {JournalWarningThresholdPerVM: {Limit: 10240, Type: 1}, JournalHardLimitPerVM: {Limit: 10240, Type: 1}}}, RecoveryVappSettings:{VCDVappSettings:{TargetVirtualDatacenter:{Id:1}}}}}}
        ...
        var selectedVmsCount = 3;
        _createVPGModel_.data = {isReverse: false};
        controller = $controller('editVcdVmController', {$scope: scope, editVcdVmFactory: factory, $translate: translate, data: data, vm: vm, selectedVmsCount:selectedVmsCount, objectTransformHelpersService: service, createVPGModel: _createVPGModel_});
    }));
    ...
    it('should have interface in place', function () {
        ...
        expect(factory.getVCDPotentialVirtualDatacenterStorageProfiles).toHaveBeenCalled();
    });
